...
import sys
import os
import Main.BackEnd.msp430.Communicator.msp430 as msp430
import Main.BackEnd.msp430.Communicator.msp430.target as target
import Main.BackEnd.msp430.Communicator.msp430.bsl5.bsl5 as bsl5
import struct
import logging
import time
import pkgutil
    ...
    class HIDBSL5(HIDBSL5Base):
        ...
        def connect(self, vid, pid):
            ...
            try:
                ...
                raise ValueError('USB VID:PID ' + vid + ':' +  pid + ' not found (not in BSL mode? or try --device)')

        def open(self, vid=DEFAULT_VID, pid=DEFAULT_PID): #"device" is string of the form VID:PID
            self.connect(vid=vid, pid=pid)
...
class HIDBSL5Target(HIDBSL5, target.Target):
