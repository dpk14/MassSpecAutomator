...
import sys
import struct
import Main.BackEnd.msp430.Communicator.msp430.memory
import Main.BackEnd.msp430.Communicator.msp430.memory.error


def load(filelike):
    """load data from a (opened) file in Intel-HEX format"""
    memory = Main.BackEnd.msp430.Communicator.msp430.memory.Memory()
    ...
    for n, l in enumerate(filelike):
        ...
        if l[0:1] != b':':
            raise Main.BackEnd.msp430.Communicator.msp430.memory.error.FileFormatError(
            ...
            if currentAddr != address:
                if segmentdata:
                    memory.segments.append(
                        Main.BackEnd.msp430.Communicator.msp430.memory.Segment(startAddr, segmentdata))
